...
# # === Helper function to group images ===
...
#     # Add overlay text
#     ax.text(10, 20, 'LightGlue Matching', color='white', fontsize=14, weight='bold', backgroundcolor='black')
#     ax.text(10, 45, f'Matches: {len(kpts0)}', color='white', fontsize=12, backgroundcolor='black')
...
# grouped = group_images_by_scene(all_images)

# print(f"Found {len(grouped)} scenes.")

# for scene, frames in grouped.items():
#     if len(frames) < 2:
#         print(f"Skipping {scene} (only {len(frames)} frame)")
#         continue
...
#     if idxs.shape[0] == 0:
#         print(f"No matches found for {scene}")
#         continue
...
#     # Draw and save
#     out_path = os.path.join(output_dir, f'{scene}_matches.png')
# # Optional: normalize match confidence if available
#     conf = matches.get('scores')  # LightGlue sometimes includes this
