...
int main()
{
    ...
    shape.setPosition({ static_cast<float>(window_res_x) / 2, static_cast<float>(window_res_y) / 2 });
    ...
    while (window.isOpen()) {
        ...
        if (futurePos.x >= minX && futurePos.x <= maxX && futurePos.y >= minY && futurePos.y <= maxY) {
            shape.move(moveVec);
        }
        ...
        view1.setSize({ static_cast<float>(view_res_x), static_cast<float>(window_res_y) });
        ...
        view.setSize({ static_cast<float>(drone_cam_res_x), static_cast<float>(drone_cam_res_y) });
