        ...
        try {
            ...
            while (true) {
                ...
                for (int i = 0; i < N_PATH; ++i) {
                    ...
                    if (i > 0 && g_cmd.path_x[i] == g_cmd.path_x[i - 1] && g_cmd.path_y[i] == g_cmd.path_y[i - 1])
                        break;
                    g_cmd.path_len = i + 1;
                }
            }
...
int main()
{
    ...
    sf::CircleShape goalMarker = makeCircle(14.f, sf::Color(255, 220, 0, 200), sf::Color::White);

    // SLAM estimated position: semi-transparent cyan circle
    sf::CircleShape slamMarker = makeCircle(10.f, sf::Color(0, 200, 255, 180), sf::Color::White);

    // ── Windows ───────────────────────────────────────────────────────────────
    sf::RenderWindow window(sf::VideoMode({ window_res_x, window_res_y }),
        "Feature Based Localization", sf::Style::Default, sf::State::Windowed);
    ...
    while (window.isOpen()) {
        ...
        if (futurePos.x >= halfSize.x && futurePos.x <= texSize2.x - halfSize.x && futurePos.y >= halfSize.y && futurePos.y <= texSize2.y - halfSize.y)
            shape.move(moveVec);
